...
function loadRegistry(): SwarmRegistryEntry[] {
  ...
  try {
    ...
    // Corrupted registry file — log and reset to empty so swarms can re-register
    console.log(chalk.dim(`[swarm-monitor] Corrupted registry at ${path}, resetting: ${(err as Error).message}`));
  }
  return [];
}
...
function saveRegistry(entries: SwarmRegistryEntry[]): void {
  ...
  try {
    ...
    console.log(chalk.dim(`[swarm-monitor] Failed to save registry to ${path}: ${(err as Error).message}`));
  }
}
...
async function notifyStall(
  ...
  if (tracked.parentIssueId) {
    try {
      ...
      console.log(chalk.dim(`[swarm-monitor] Failed to post stall notice on ${tracked.parentIssueKey ?? tracked.parentIssueId}: ${(err as Error).message}`));
    }
  }
}
...
async function notifyCompletion(
  ...
  if (tracked.parentIssueId) {
    try {
      ...
      console.log(chalk.dim(`[swarm-monitor] Failed to post completion report on ${tracked.parentIssueKey ?? tracked.parentIssueId}: ${(err as Error).message}`));
    }
  }
}
...
async function postProgressComment(
  ...
  try {
    ...
    console.log(chalk.dim(`[swarm-monitor] Failed to post progress update on ${tracked.parentIssueKey ?? tracked.parentIssueId}: ${(err as Error).message}`));
  }
}
...
export function getSwarmDashboardData(): SwarmDashboardData[] {
  ...
  for (const entry of registry) {
    try {
      ...
      console.log(chalk.dim(`[swarm-monitor] Skipping broken registry entry ${entry.workspacePath}: ${(err as Error).message}`));
    }
  }
