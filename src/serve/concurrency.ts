...
import {
  suspendSession, resumeSessionProcess, sessionExists, killSession, sendKeys,
} from '../core/tmux.js';
...
// RYA-1184: the Claude session limit is shared account-wide. On 2026-06-10 an
// outage-recovery wave spawned 7 concurrent sessions in ~10 min, exhausted the
// shared limit, and EVERY session froze on an interactive billing dialog for 7h
// (no agent can answer a billing prompt). This cap is the last line of defense:
// every spawn path funnels through agentStartCommand's canStartNewSession()
// gate, so overflow queues (or hibernates an older session) instead of spawning.
// Fail closed on a malformed override: NaN would make every `count >= cap`
// comparison false and silently disable the gate.
const envMaxSessions = parseInt(process.env.AOS_MAX_SESSIONS || '', 10);
export const GLOBAL_MAX_SESSIONS =
  Number.isInteger(envMaxSessions) && envMaxSessions > 0 ? envMaxSessions : 3;

// Default per-role limits (overridden by config.json maxParallel).
// Tightened 2026-05-06 — Claude Code per-account quota saturates at ~4 active
// agents. Higher per-role caps are pointless when the global cap is 4.
...
/** Parse ISO timestamp to epoch ms, normalizing missing Z suffix from SQLite */
...
export function getMaxParallel(role: string): number {
  ...
  } catch (err) { log.warn('Failed to load agent config for max parallel', { error: (err as Error).message }); }
...
export function pickToEvict(role: string, excludeIssueKey?: string): EvictResult | null {
  ...
  // RYA-1312: only sessions whose HANDOFF is already written may be KILLED —
  // "idle" just means paused-at-prompt, and reclaiming an idle session that
  // hasn't written HANDOFF.md destroys its in-flight work (RYA-1302/RYA-1307
  // were killed mid-task this way on 2026-07-20, leaving In Review issues
  // with empty state dirs). Unfinished idle sessions hibernate instead:
  // frees the slot the same, loses nothing.
  const aliveIdle = idleSessions
    .filter(a => a.issue_key !== excludeIssueKey && a.tmux_session && sessionExists(a.tmux_session))
    .sort((a, b) => parseTimestamp(a.updated_at) - parseTimestamp(b.updated_at));
  ...
  if (preferSameRole) {
    ...
    if (sameRoleIdle.length > 0 && hibernateSession(sameRoleIdle[0].id)) {
      return { type: 'suspended', attempt: sameRoleIdle[0] };
    }
  }
  ...
  if (aliveIdle.length > 0 && hibernateSession(aliveIdle[0].id)) {
    return { type: 'suspended', attempt: aliveIdle[0] };
  }
  ...
  if (preferSameRole) {
    ...
    const eligible = sameRoleRunning.filter(a => {
      const age = now - parseTimestamp(a.created_at);
...
export function reclaimSession(attempt: Attempt, reason: string): void {
  if (attempt.tmux_session && sessionExists(attempt.tmux_session)) {
    try { killSession(attempt.tmux_session); } catch (err) { log.warn('Failed to kill session during reclaim', { error: (err as Error).message }); }
  }
...
export function selectSessionToHibernate(excludeIssueKey?: string): Attempt | null {
  ...
  eligible.sort((a, b) => {
    ...
    return parseTimestamp(a.created_at)
         - parseTimestamp(b.created_at);
  });
...
/** SIGSTOP a running or idle session (RYA-1312: idle = paused-at-prompt, still has unfinished work). */
export function hibernateSession(attemptId: string): boolean {
  const attempt = [...getActiveAttempts(), ...getIdleAttempts()].find(a => a.id === attemptId);
  if (!attempt || !attempt.tmux_session || (attempt.status !== 'running' && attempt.status !== 'idle')) return false;
...
export function wakeSession(attemptId: string): boolean {
  ...
  try {
    ...
    // RYA-1312: Re-anchor the agent to its task after SIGCONT. While frozen it
    // cannot read the pty, so anything sent to the pane during hibernation
    // (Discord relays, monitor nudges) is consumed in a burst on wake and can
    // derail the conversation. Injecting the issue key after SIGCONT makes the
    // agent re-anchor regardless of what queued — and the no-task quality gate
    // (quality-gate.ts) catches and re-delivers if a bogus HANDOFF is written.
    const { tmux_session, issue_key, agent_type } = attempt;
    setTimeout(() => {
      try {
        if (!sessionExists(tmux_session!)) return;
        sendKeys(tmux_session!,
          `[SYSTEM WAKE] Your task is still ${issue_key} — you were hibernated to free capacity. ` +
          `Ignore any messages you may have seen during hibernation and continue working on ${issue_key}. ` +
          `If your work on ${issue_key} is already complete, write HANDOFF.md now.`);
        log.debug('Wake: injected task context', { role: agent_type, issueKey: issue_key });
      } catch (err) {
        log.debug('Wake: task context injection failed (best-effort)', { issueKey: issue_key, error: (err as Error).message });
      }
    }, 500);
