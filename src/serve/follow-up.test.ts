...
describe('spawnFollowUp', () => {
  ...
  it('does not emit activity — session ack handled by caller (RYA-286)', async () => {
    await spawnFollowUp({ ...baseOpts, agentSessionId: 'session-123' });

    // emitActivity is no longer called from spawnFollowUp.
    // Session acknowledgment (dismissAgentSession) is handled by the caller (webhook.ts).
  ...
  it('does not track followUpMeta when no commentId and no Discord context', async () => {
    const result = await spawnFollowUp(baseOpts);
  ...
  it('tracks followUpMeta with Discord context', async () => {
    ...
    expect(result).not.toBeNull();
    expect(followUpMeta.has(result!.attemptId)).toBe(true);
    const meta = followUpMeta.get(result!.attemptId)!;
    expect(meta.discordChannelId).toBe('ch-123');
    expect(meta.discordMessageId).toBe('msg-456');
    expect(meta.commentId).toBeUndefined();
  });

  it('includes Discord reply instructions in prompt when Discord context present', async () => {
    await spawnFollowUp({
      ...baseOpts,
      discordChannelId: 'ch-123',
      discordMessageId: 'msg-456',
    ...
    const adapter = (getAdapter as ReturnType<typeof vi.fn>).mock.results[0].value;
    const prompt = adapter.spawn.mock.calls[0][0].initialPrompt;
    expect(prompt).toContain('discord-reply');
    expect(prompt).toContain('ch-123');
    expect(prompt).toContain('msg-456');
    expect(prompt).toContain('message on Discord');
  });
