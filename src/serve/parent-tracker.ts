...
export async function checkParentCompletion(parentKey: string): Promise<void> {
  ...
  try {
    ...
    if (status.allDone && status.total > 0) {
      // Use the most recent agent's token so changes aren't attributed to the CEO
      const parentAttempts = getAttemptsByIssue(parentKey);
      const lastAgent = parentAttempts.find(a => a.agent_type);
      const parentToken = lastAgent ? (getAgentLinearToken(lastAgent.agent_type) || undefined) : undefined;

      // RYA-1116: Honor the parent agent's sticky in-progress intent.
      // If the parent's own HANDOFF.md explicitly chose `status_intent: in-progress`
      // (or `no-change`), sub-issue completion alone must not promote the parent.
      // The parent agent has to be re-dispatched and rewrite HANDOFF.md to release.
      if (lastAgent?.workspace_path) {
        const parentHandoff = readFileOnRemote(
          resolveStatePath(parentKey, lastAgent.workspace_path, 'HANDOFF.md')
        );
        if (hasStickyInProgressIntent(parentHandoff)) {
          console.log(chalk.dim(`  Parent ${parentKey}: all ${status.total} subs done but parent has sticky status_intent — leaving In Progress (parent agent must rewrite HANDOFF.md to release)`));
          // Mark as closed so we don't keep re-checking and spamming the log.
          // Re-checking is unnecessary until the parent's HANDOFF is rewritten,
          // which will trigger the main completion flow with its own state update.
      ...
      try {
        await updateIssueState(parentIssue.id, 'In Review', parentToken);
      } catch (err) {
      ...
      try {
        ...
        console.log(chalk.dim(`[parent-tracker] completion comment for ${parentKey} failed: ${(err as Error).message}`));
      }
