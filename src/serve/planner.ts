...
export function parsePlannerResponse(llmResponse: string, issuePriority: number, parentIssueKey: string): PlanResult {
      ...
      if (Array.isArray(parsed)) {
        subtasks = parsed
          ...
          .map((t: Record<string, unknown>) => ({
            ...
            priority: typeof t.priority === 'number' ? t.priority : issuePriority,
          }));
      }
    } catch (parseErr) {
...
export async function runPlanningCli(fullPrompt: string, workspacePath: string): Promise<PlannerCliResult> {
  const { spawnSync } = await import('child_process');

  // Pin the planner model explicitly: the CLI default (fable-5) is 2x the
  // price of opus-4-8, which handles decomposition fine. AOS_PLANNER_MODEL
  // overrides, mirroring the grader/dream knobs.
  const plannerModel = process.env.AOS_PLANNER_MODEL || 'claude-opus-4-8';
  const claudeResult = spawnSync('claude', ['-p', '--output-format', 'text', '--model', plannerModel], {
    input: fullPrompt,
    encoding: 'utf-8',
    timeout: 120_000,
    maxBuffer: 1024 * 1024,
  ...
  if (!claudeResult.error && claudeResult.status === 0 && claudeOutput) {
    return { runner: 'claude', output: claudeOutput };
  }

  const codexTempDir = mkdtempSync(join(tmpdir(), 'agentos-planner-'));
  const outputPath = join(codexTempDir, 'last-message.txt');
  ...
  try {
    ...
    const codexOutput = ((existsSync(outputPath) ? readFileSync(outputPath, 'utf-8') : '') || codexResult.stdout || '').trim();
    if (!codexResult.error && codexResult.status === 0 && codexOutput) {
      return { runner: 'codex', output: codexOutput };
    }

    const claudeErr = claudeResult.error?.message || claudeResult.stderr?.trim() || `exit ${claudeResult.status}`;
    const codexErr = codexResult.error?.message || codexResult.stderr?.trim() || `exit ${codexResult.status}`;
    throw new Error(`Planning LLM failed via claude (${claudeErr}) and codex (${codexErr})`);
  } finally {
...
/**
 * Use the configured local coding CLIs to decompose a parent issue into sub-tasks.
...
Current status: ${issue.state}`;
  ...
  // Combine system prompt + user prompt into a single prompt for the planner runner.
  const fullPrompt = `${systemPrompt}\n\n---\n\n${userPrompt}`;
  const llmResponse = await runPlanningCli(fullPrompt, workspacePath);
  if (llmResponse.runner === 'codex') {
    console.log(chalk.yellow(`  Planner fallback: Claude unavailable, used Codex for ${issue.identifier}`));
  }

  return parsePlannerResponse(llmResponse.output, issue.priority || 2, issue.identifier);
}
      ...
      if (issue) {
        ...
        if (assigneeId) {
          try {
            ...
            console.log(chalk.dim(`  [planner] delegate update failed for ${issue.identifier}: ${(err as Error).message}`));
          }
        }
...
export async function planAndDispatch(issueKey: string): Promise<PlanAndDispatchResult> {
  ...
  try {
    ...
    console.log(chalk.dim(`  [planner] dispatch summary comment failed for ${parentIssue.identifier}: ${(err as Error).message}`));
  }
