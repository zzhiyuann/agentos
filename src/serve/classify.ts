...
export function routeEvent(
  ...
  switch (classification) {
    ...
    case 'issue-created': {
      ...
      // Check for "Swarm" label — triggers swarm optimization (priority over Plan)
      if (labels.some(l => l.name.toLowerCase() === 'swarm')) {
        return { classification, action: 'conditional-spawn', targetAgent: 'swarm-trigger', reason: 'Issue has "Swarm" label — trigger swarm optimization' };
      }
