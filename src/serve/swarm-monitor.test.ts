...
beforeEach(() => {
  ...
  // Point HOME to test workspace so swarm registry doesn't conflict with other tests
  process.env.HOME = TEST_WORKSPACE;
  const aosDir = join(TEST_WORKSPACE, '.aos');
  mkdirSync(aosDir, { recursive: true });
  writeFileSync(join(aosDir, 'swarm-registry.json'), '[]');
});
