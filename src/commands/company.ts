...
const COMPANY_DIR = join(STATE_DIR, 'company');
  ...
  for (const [role, duties] of Object.entries(responsibilities)) {
    ...
    for (const duty of allDuties) {
      ...
      try {
        ...
        const desc = `Standing duty for ${role}.\n\n## Action\n\n${duty.action}\n\n## Instructions\n\n- Complete the action described above\n- Post findings to Discord: \`AGENT_ROLE=${role} linear-tool group "summary of findings"\`\n- Anything actionable must be a dispatched sub-issue (Findings → Action protocol) — never prose recommendations\n- Write HANDOFF.md with \`status_intent: done\` — standing-duty reports are info-only; the CEO gets them via the Discord summary and sub-issues, so this issue must NOT go to In Review\n- Exit with /exit`;
