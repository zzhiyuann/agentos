...
import {
  ...
  dismissAgentSession, generateHandoffSummary, updateIssueState,
} from '../core/linear.js';
...
async function pollCycle(): Promise<void> {
  ...
  // 1. Auto-spawn: issues labeled with any registered agent runner in Todo state
  let totalQueuedIssues = 0;
  const registry = getAgentRegistry();
  for (const [agentType, def] of Object.entries(registry)) {
    const todoIssues = await getIssuesByLabel(def.label, WORKFLOW_STATES.TODO);
    totalQueuedIssues += todoIssues.length;
    for (const issue of todoIssues) {
      if (!getActiveAttempt(issue.identifier)) {
        console.log(chalk.cyan(`[${ts}] Auto-spawning: ${issue.identifier} - ${issue.title} [${agentType}]`));
        try {
          await spawnCommand(issue.identifier, { agent: agentType });
        } catch (err: unknown) {
  ...
  if (activeCount > 0 || totalQueuedIssues > 0) {
    console.log(chalk.dim(`[${ts}] Active: ${activeCount} | Queued: ${totalQueuedIssues}`));
  }
}
...
async function handleSessionEnd(attempt: ReturnType<typeof getActiveAttempts>[0]): Promise<void> {
  ...
  const agentToken = getAgentLinearToken(attempt.agent_type) || undefined;
  ...
  if (handoff) {
    ...
    const docUrl = await createIssueDocument(attempt.issue_id, `Handoff: Attempt #${attempt.attempt_number}`, handoff, agentToken);
    ...
    if (hasAgentAccess() && attempt.agent_session_id) {
      ...
      await addComment(attempt.issue_id, `**Agent completed** (Attempt #${attempt.attempt_number})\n\n${summary}${docNote}`, agentToken);
    }

    try {
      await updateIssueState(attempt.issue_id, WORKFLOW_STATES.IN_REVIEW, agentToken);
    } catch { /**/ }
    ...
    if (hasAgentAccess() && attempt.agent_session_id) {
      ...
      try { await addLabelToIssue(attempt.issue_id, AGENT_LABELS.BLOCKED, agentToken); } catch { /**/ }
    }
