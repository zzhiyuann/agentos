...
describe('routeEvent', () => {
  ...
  // Requires ~/.aos/agents/ to build the agent role regex at module load time
  describeLocal('@mention pipe events', () => {
  ...
  describeLocal('@mention spawn events', () => {
...
describe('serve - HTTP endpoints', () => {
  ...
  it('ping endpoint returns uptime, activeAgents, and timestamp', async () => {
    try {
      const res = await fetch('http://localhost:3848/ping');
      if (res.ok) {
        const data = await res.json();
        expect(data.status).toBe('ok');
