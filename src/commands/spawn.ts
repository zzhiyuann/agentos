...
import {
  ...
  updateAgentPlan, addExternalLink,
} from '../core/linear.js';
...
import { agentExists, loadAgentConfig, getAgentLinearToken } from '../core/persona.js';
...
export async function spawnCommand(issueKeyOrId: string, options: { agent?: string; agentSessionId?: string; followUpPrompt?: string }): Promise<void> {
  ...
  // 8. Post to Linear — use per-agent token so changes aren't attributed to the CEO
  const spawnToken = getAgentLinearToken(agentRole) || undefined;
  if (hasAgentAccess() && agentSessionId) {
    // Agent session is visible in Linear UI — no comment needed
  } else {
    await addComment(issue.id, `**Agent started** (\`${agentType}\`) — \`aos jump ${issue.identifier}\``, spawnToken);
  }

  // 9. Move issue to In Progress (use per-agent token)
  try {
    await updateIssueState(issue.id, WORKFLOW_STATES.IN_PROGRESS, spawnToken);
  } catch (err) {
