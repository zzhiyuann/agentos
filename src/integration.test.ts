...
/** Tests that need local ~/.aos/ environment (agents, routing, shared-memory) */
...
describeLocal('Integration: Per-agent OAuth tokens', () => {
...
describeLocal('Integration: Workspace mapping', () => {
...
describeLocal('Integration: System config consistency', () => {
  ...
  // A3.2: shared boilerplate (Linear Tools / Findings → Action Protocol) was
  // extracted to templates/agent-common.md and is injected by
  // buildGroundingPrompt — matching sections are stripped from claudeMd at
  // load time, so assert against the assembled grounding prompt instead.
  it('all agents have Findings → Action Protocol in their grounding prompt', async () => {
    const { buildGroundingPrompt } = await import('./core/persona.js');
    for (const role of listAgents()) {
      const persona = loadPersona(role);
      const grounding = buildGroundingPrompt(persona, 'task');
      expect(grounding).toContain('Findings');
      expect(grounding).toContain('Action Protocol');
    }
  });

  it('all agents have Linear tools section in their grounding prompt', async () => {
    const { buildGroundingPrompt } = await import('./core/persona.js');
    for (const role of listAgents()) {
      const persona = loadPersona(role);
      const grounding = buildGroundingPrompt(persona, 'task');
      expect(grounding).toContain('Linear Tools');
    }
  });
