    ...
    for (const s of sessions) {
      if (
        ...
        try {
          ...
          console.debug(`[linear-sessions] dismiss session ${s.id} failed:`, (err as Error).message);
        }
      }
    }
    return closed;
...
/** Check if an AgentSession is already complete on Linear's side.
 *  Survives serve restarts (unlike globalDismissedSessions which is in-memory). */
async function isSessionAlreadyComplete(agentSessionId: string, token: string): Promise<boolean> {
  try {
    const data = await graphql(token, `
      query($id: String!) { agentSession(id: $id) { status } }
    `, { id: agentSessionId });
    const status = (data.agentSession as { status?: string } | null)?.status;
    return status === 'complete';
  } catch {
    return false; // If query fails, assume not complete (safe to try dismiss)
  }
}
 ...
 *  RYA-312: Also checks Linear API to prevent double-dismiss after serve restart. */
export async function dismissAgentSession(agentSessionId: string, overrideToken?: string, reason?: string): Promise<void> {
  if (globalDismissedSessions.has(agentSessionId)) return; // Already dismissed in this process — skip
  ...
  // RYA-312: Check Linear API — prevents double-dismiss after serve restart
  // (globalDismissedSessions is in-memory, lost on restart)
  if (await isSessionAlreadyComplete(agentSessionId, token)) {
    globalDismissedSessions.add(agentSessionId); // Cache for future calls
    return;
  }

  // Sanitize dismiss body: never show bare '–' to users (RYA-180, RYA-301).
  // Response activities are ALWAYS visible in Linear timeline — use meaningful text.
  let body = reason && reason.trim() ? reason.trim() : 'Session complete.';
  if (body === '–' || body === '-') body = 'Session complete.';
