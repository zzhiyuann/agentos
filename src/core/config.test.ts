...
import { writeFileSync, existsSync, mkdirSync, rmSync, readFileSync, renameSync } from 'fs';
...
describe('getConfig', () => {
  ...
  it('dbPath defaults to ~/.aos/state.db when AOS_DB_PATH is unset', () => {
    const saved = process.env.AOS_DB_PATH;
    delete process.env.AOS_DB_PATH;
    try {
      expect(getConfig().dbPath).toContain('.aos/state.db');
    } finally {
      if (saved !== undefined) process.env.AOS_DB_PATH = saved;
    }
  });

  it('dbPath honours the AOS_DB_PATH override (test sqlite isolation, RYA-1198)', () => {
    const saved = process.env.AOS_DB_PATH;
    process.env.AOS_DB_PATH = '/tmp/aos-test-override/state.db';
    try {
      expect(getConfig().dbPath).toBe('/tmp/aos-test-override/state.db');
    } finally {
      if (saved === undefined) delete process.env.AOS_DB_PATH;
      else process.env.AOS_DB_PATH = saved;
    }
  });
