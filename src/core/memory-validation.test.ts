...
- [Old notes](memory/old-notes.md)`,
  ...
  it('exempts nightly reflections files from the unindexed check', () => {
    setMemoryFiles('cto', {
      'architecture.md': '# Arch',
      'reflections-2026-06-11.md': '# Reflection',
    });
    setMemoryIndex('cto', '- [Architecture](memory/architecture.md)');
    ...
    expect(result.unindexedFiles).toEqual([]);
    expect(result.warnings).toEqual([]);
    // Still counted as a memory file (zero-memory check unaffected)
    expect(result.memoryFileCount).toBe(2);
  });
