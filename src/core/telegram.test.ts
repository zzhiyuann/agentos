...
describe('parseMentions', () => {
  ...
  it('parses bot-prefixed mentions when AOS_TELEGRAM_BOT_PREFIX is set', () => {
    const orig = process.env.AOS_TELEGRAM_BOT_PREFIX;
    process.env.AOS_TELEGRAM_BOT_PREFIX = 'myorg_';
    try {
      expect(parseMentions('hey @myorg_cto')).toEqual(['cto']);
    } finally {
      if (orig === undefined) delete process.env.AOS_TELEGRAM_BOT_PREFIX;
      else process.env.AOS_TELEGRAM_BOT_PREFIX = orig;
    }
  });

  it('skips bot-prefixed mentions when AOS_TELEGRAM_BOT_PREFIX is unset', () => {
    const orig = process.env.AOS_TELEGRAM_BOT_PREFIX;
    delete process.env.AOS_TELEGRAM_BOT_PREFIX;
    try {
      expect(parseMentions('hey @someprefix_cto')).toEqual([]);
    } finally {
      if (orig !== undefined) process.env.AOS_TELEGRAM_BOT_PREFIX = orig;
    }
  });
