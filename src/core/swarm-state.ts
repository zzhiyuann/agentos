...
export class SwarmStateManager {
  ...
  getExperiments(): Experiment[] {
    ...
    return readdirSync(dir)
      ...
      .flatMap(f => {
        try {
          return [this.readJson(`experiments/${f}`)];
        } catch {
          console.warn(`[swarm-state] Skipping corrupted experiment file: ${f}`);
          return [];
        }
      })
  ...
  releaseLock(experimentId: string): void {
    ...
    try {
      ...
      console.debug(`[swarm] lock release failed for ${experimentId}:`, (err as Error).message);
    }
  }
