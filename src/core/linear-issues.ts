...
export async function getIssue(idOrKey: string): Promise<LinearIssueInfo> {
  ...
  if (idOrKey.includes('-')) {
    ...
    try {
      ...
      // Linear SDK error (rate limit, network) — surface as transient, not "not found"
      throw new Error(`Issue ${idOrKey} lookup failed (transient): ${(err as Error).message}`);
    }
    if (issues.nodes.length === 0) {
      // Empty result is ambiguous: either the issue truly doesn't exist, or
      // Linear quietly degraded the response under rate-limit pressure. Fall
      // back to the by-identifier endpoint, which throws cleanly on rate-limit.
      try {
        issue = await client.issue(idOrKey);
      } catch (err) {
        const msg = (err as Error).message || '';
        if (/rate limit|RATELIMITED|429/i.test(msg)) {
          throw new Error(`Issue ${idOrKey} lookup rate-limited (transient): ${msg}`);
        }
        throw new Error(`Issue ${idOrKey} not found (was deleted)`);
      }
    } else {
...
export async function addLabelToIssue(issueId: string, labelName: string, agentToken?: string): Promise<void> {
  const client = agentToken ? new LinearClient({ accessToken: agentToken }) : getReadClient();
...
export async function removeLabelFromIssue(issueId: string, labelName: string, agentToken?: string): Promise<void> {
  const client = agentToken ? new LinearClient({ accessToken: agentToken }) : getReadClient();
  const issue = await client.issue(issueId);
  const existingLabels = await issue.labels();
...
export async function createIssueDocument(issueId: string, title: string, content: string, agentToken?: string): Promise<string | null> {
  try {
    // Use agent OAuth token if provided, otherwise fall back to OAuth system or personal API key
    const token = agentToken
      ? `Bearer ${agentToken}`
      : (hasAgentAccess() ? `Bearer ${getOAuthToken()!}` : getLinearApiKey());
...
/** Create a resource attachment on a Linear issue (shows in sidebar Resources) */
export async function createIssueAttachment(issueId: string, url: string, title: string, subtitle?: string, agentToken?: string): Promise<boolean> {
  try {
    const token = agentToken
      ? `Bearer ${agentToken}`
      : (hasAgentAccess() ? `Bearer ${getOAuthToken()!}` : getLinearApiKey());
    ...
    const result = data.attachmentCreate as { success: boolean } | undefined;
    return result?.success ?? false;
  } catch (err) {
    console.error('Failed to create attachment:', (err as Error).message);
    return false;
  }
}
 ...
 * - BRAND-PLAYBOOK.md (bare uppercase .md filenames after bullet/number)
 * - `BRAND-PLAYBOOK.md` (backtick-wrapped — common in HANDOFF "Files Changed" sections)
 * - presentation.html (HTML deliverables, any case)
 * - `presentation.html` (backtick-wrapped HTML)
...
export async function linkifyDeliverables(
  ...
  // Match deliverable filenames in HANDOFF text:
  // - UPPERCASE-NAME.md (with or without backtick wrapping, optional ./ prefix)
  // - *.html files, any case (with or without backtick wrapping)
  ...
  while ((match = fileRefPattern.exec(text)) !== null) {
    const fullRef = match[1]; // e.g., ./BRAND-PLAYBOOK.md or presentation.html
    const filename = match[2]; // e.g., BRAND-PLAYBOOK.md or presentation.html
    ...
    try {
      ...
      const title = filename.replace(/\.(md|html)$/i, '').replace(/[-_]/g, ' ');

      // For HTML files, wrap in a markdown code block so content is readable in Linear
      let uploadContent = content;
      if (ext === 'html') {
        uploadContent = `# ${title}\n\n> HTML file — open in a browser for full rendering.\n\n\`\`\`html\n${content}\n\`\`\``;
      }

      const url = await createIssueDocument(issueId, title, uploadContent, agentToken);
      if (url) {
        replacements.push({ original: filename, filename, url });
        // Also add as issue attachment (shows in Resources sidebar)
        await createIssueAttachment(issueId, url, `📄 ${filename}`, ext === 'html' ? 'HTML presentation' : 'Document', agentToken);
      }
    } catch {
  ...
  // Apply replacements: turn bare/backtick-wrapped filenames into markdown links
  let result = text;
  for (const { original, filename, url } of replacements) {
    // First: replace backtick-wrapped references `filename.ext` → [📄 filename.ext](url)
    const backtickPattern = new RegExp(
      `\`${escapeRegex(original)}\``,
      'g'
    );
    result = result.replace(backtickPattern, `[📄 ${filename}](${url})`);

    // Then: replace standalone bare references (not already inside a markdown link)
