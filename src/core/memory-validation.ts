...
export function validatePostSessionMemory(
  ...
  for (const file of memoryFiles) {
    // Nightly dream.ts reflections are dated journal artifacts written by
    // automation with no index pass — exempt them from the index requirement
    // (dream.ts likewise excludes them when reading memories).
    if (file.startsWith('reflections-')) continue;
