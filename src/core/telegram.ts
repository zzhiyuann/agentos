...
const CONFIG_PATH = join(STATE_DIR, 'telegram.json');
...
export function parseMentions(text: string): string[] {
  ...
  const botPrefix = process.env.AOS_TELEGRAM_BOT_PREFIX;
  for (const [pattern, role] of Object.entries(roleMap)) {
    if (lower.includes(`@${pattern}`) || (botPrefix && lower.includes(`@${botPrefix}${pattern}`))) {
