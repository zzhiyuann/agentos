...
// --- Rate limiter for custom GraphQL calls ---
// AGENTOS_LINEAR_RATE_LIMIT_RPS caps raw graphql() calls (default: 10/s).
// SDK calls (LinearClient.updateIssue, etc.) are not covered here — see ensureDelegate debounce.
let _lastGraphqlMs = 0;
function graphqlMinIntervalMs(): number {
  const rps = parseFloat(process.env.AGENTOS_LINEAR_RATE_LIMIT_RPS ?? '10');
  return isNaN(rps) || rps <= 0 ? 0 : Math.ceil(1000 / rps);
}

// --- Raw GraphQL helper ---

export async function graphql(token: string, query: string, variables?: Record<string, unknown>): Promise<Record<string, unknown>> {
  // Enforce AGENTOS_LINEAR_RATE_LIMIT_RPS between raw API calls
  const interval = graphqlMinIntervalMs();
  if (interval > 0) {
    const elapsed = Date.now() - _lastGraphqlMs;
    if (elapsed < interval) {
      await new Promise(r => setTimeout(r, interval - elapsed));
    }
    _lastGraphqlMs = Date.now();
  }
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    // Auth error — try refreshing the global token and retry once
    if (res.status === 401 && attempt === 0) {
      console.warn('[linear] 401 Unauthorized — attempting token refresh');
      try {
        const client = await refreshAgentClient();
        // Use the freshly refreshed global token for the retry
        const freshToken = getOAuthToken();
        if (freshToken) {
          currentToken = freshToken.startsWith('Bearer ') ? freshToken : `Bearer ${freshToken}`;
        }
      } catch (err: unknown) {
        console.warn(`[linear] token refresh failed:`, (err as Error).message);
      }
