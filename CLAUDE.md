...
scripts/install-hooks.sh  # install git pre-commit + post-commit hooks (run once per clone)
```

### Git hooks (RYA-619)

Every clone must run `scripts/install-hooks.sh` once. This installs a **pre-commit hook** that runs the same checks CI does — `tsc --noEmit`, `npm run lint`, `npx vitest run --retry=2` — so broken state never gets committed locally. The hook skips when a commit touches no `src/`/config files.

**Do not use `--no-verify`** to bypass the hook unless the CEO has explicitly approved it for a specific commit. If the hook fails, fix the underlying issue. If it fails on a flaky test unrelated to your change, file a follow-up issue for the flake before bypassing.

Background: RYA-614's broken main came from 19 unpushed commits that never hit CI (CI runs on push/PR only). The pre-commit hook closes that gap.
