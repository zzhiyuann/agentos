...
# RYA-1195: build through the shared mutex so this hook and the auto-deploy
# watcher inside serve never run tsc concurrently (deploy-gap variant #4).
scripts/build-locked.sh 2>&1 | tail -5
